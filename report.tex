...
The selected top-5 heads are listed in Table~\ref{tab:top5_heads}. All five heads are located in layers 11--18, consistent with SAPLMA's optimal layer range (Layer 18).
...
SAPLMA-L18       & .700 & .667 & .621 & .643 & .765 \\
\midrule
Single L16\_H6   & .710 & .688 & .609 & .646 & .754 \\
Single L15\_H6   & .690 & .640 & .655 & .648 & .739 \\
Single L11\_H1   & .720 & .707 & .609 & .654 & .746 \\
Single L18\_H0   & .635 & .567 & .678 & .618 & .717 \\
Single L13\_H11  & .685 & .662 & .563 & .609 & .763 \\
\midrule
Summed-Heads     & \textbf{.735} & .681 & \textbf{.736} & \textbf{.707} & \textbf{.799} \\
Voting Ensemble  & .725 & \textbf{.722} & .598 & .654 & .795 \\
\bottomrule
\end{tabular}
\caption{End-to-end hallucination detection on the TriviaQA test set (N=200). The Summed-Heads probe achieves the best AUC (0.799) and Accuracy (0.735).}
...
Key observations:
\begin{itemize}
    \item The \textbf{Summed-Heads probe} (AUC 0.799) surpasses SAPLMA (AUC 0.765) using only 5 of 336 heads' influence vectors, demonstrating that head-level features can be more discriminative than the full hidden state.
    \item \textbf{Feature-level fusion} (Summed-Heads, AUC 0.799) slightly outperforms \textbf{decision-level fusion} (Voting, AUC 0.795), suggesting the head signals are complementary at the feature level.
    ...
    \item Among single-head probes, \textbf{L11\_H1} achieves the highest accuracy (0.720) while \textbf{L13\_H11} achieves the highest single-head AUC (0.763), approaching the full SAPLMA baseline.
\end{itemize}
...
The slight advantage of Summed-Heads (AUC 0.799) over Voting (AUC 0.795) suggests that the top-5 heads capture complementary aspects of the truth/hallucination distinction, and that combining their signals at the feature level allows the probe to learn richer joint patterns than majority voting at the decision level.
...
\begin{enumerate}
    ...
    \item A novel per-head probe method, using only 5 of 336 attention heads' influence vectors, achieves AUC 0.799 on a TriviaQA test set---surpassing the full SAPLMA baseline (AUC 0.765) with 300$\times$ fewer features.
