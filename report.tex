...
\begin{enumerate}
    ...
    \item \textbf{Representation Analysis} (Section~\ref{sec:analysis}): We investigate how layer depth and token position affect detection quality, confirming an inverted-U performance curve peaking at Layer 16 and demonstrating that the last token position is critical for detection.
    \item \textbf{Per-Head Probe Method} (Section~\ref{sec:headprobe}): We propose a novel approach that trains independent binary classifiers on individual attention heads' influence vectors, selects the top-5 most discriminative heads by validation AUC, and combines them via a voting ensemble. This method achieves AUC 0.794 on a TriviaQA test set using only 5 of 336 heads, surpassing the full SAPLMA baseline.
\end{enumerate}
...
SAPLMA \citep{azaria2023internal} extracts the last token's hidden state from intermediate layers and trains a supervised probe. We extract embeddings from layers 14, 16, 18, 20, 22, 24, 26, and 28 (every 2 layers from the middle to the last layer) and train a three-layer feedforward probe:
\begin{equation}
...
We evaluate SAPLMA probes at layers 14, 16, 18, 20, 22, 24, 26, and 28 to characterize how detection quality varies with depth. This reveals the optimal layer for extracting truth-discriminative representations.

\subsubsection{Token Position Analysis}

At Layer 16 (the optimal layer from Stage 1), we compare probe performance using hidden states from four token positions:
\begin{itemize}
...
On the test set (Set C, 200 samples), we evaluate multiple fusion strategies:
...
Table~\ref{tab:saplma_layer} presents SAPLMA results across eight layers. Layer 16 achieves the best average performance (Accuracy 0.751, AUC 0.843), while the final layer (28) performs worst (AUC 0.756), exhibiting an inverted-U pattern.
...
\midrule
14 & .925 & .851 & .737 & .773 & .911 & .806 \\
16 & .909 & .864 & .710 & .787 & .924 & .861 \\
18 & .912 & .864 & .694 & .754 & .913 & .851 \\
20 & .933 & .859 & .749 & .758 & .897 & .844 \\
22 & .887 & .769 & .760 & .769 & .875 & .837 \\
24 & .867 & .782 & .763 & .723 & .850 & .827 \\
26 & .844 & .761 & .740 & .717 & .808 & .801 \\
28 & .816 & .742 & .751 & .699 & .742 & .788 \\
\midrule
\textbf{Avg} & .887 & .811 & .738 & .748 & .865 & .827 \\
\bottomrule
\end{tabular}
\caption{SAPLMA AUC by layer and topic. Layer 16 achieves the highest average AUC (0.843). Values shown are average AUC across 3 random restarts with leave-one-out cross-validation.}
...
\begin{figure}[t]
    ...
    \caption{PPL vs SAPLMA (Layer 16) comparison across six topics. SAPLMA consistently outperforms PPL in both Accuracy and AUC.}
...
Figure~\ref{fig:layer_analysis} shows how SAPLMA performance varies with layer depth. The left panel confirms the inverted-U pattern, with Layer 16 (57\% depth) achieving the best average AUC. The right panel shows this pattern holds across most topics, though the exact peak varies slightly.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{processed_datasets/fig_report_2_layer_analysis.png}
    \caption{Left: Average Accuracy and AUC across layers. Right: Per-topic AUC across layers. The inverted-U pattern peaks at Layer 16.}
    \label{fig:layer_analysis}
\end{figure}

Table~\ref{tab:token_position} and Figure~\ref{fig:token_position} present the token position analysis at Layer 16. The last token dramatically outperforms all other positions (AUC 0.844), while the first and second tokens are near random (AUC $\sim$0.50). The second-to-last token shows intermediate performance (AUC 0.690), suggesting a gradual accumulation of semantic information.
...
First           & 0.504 & 0.500 \\
Second          & 0.504 & 0.493 \\
Second-to-Last  & 0.621 & 0.690 \\
\textbf{Last}   & \textbf{0.751} & \textbf{0.844} \\
\bottomrule
\end{tabular}
\caption{Token position analysis at Layer 16. The last token achieves dramatically superior performance.}
...
\begin{figure}[t]
    ...
    \caption{Left: Per-topic AUC by token position at Layer 16. Right: Average performance by position. The last token is critical for hallucination detection.}
...
We trained 336 independent binary classifiers (one per attention head) on Set A and evaluated their AUC on Set B. Figure~\ref{fig:head_heatmap} shows the AUC distribution across all heads. High-performing heads concentrate in layers 11--18, with the top head (L16\_H6) achieving AUC 0.731.
...
The selected top-5 heads are listed in Table~\ref{tab:top5_heads}. All five heads are located in layers 11--18, consistent with SAPLMA's optimal layer range (Layer 16).
...
\bottomrule
\end{tabular}
\caption{Top-5 attention heads selected by validation AUC. All heads are in layers 11--18.}
\label{tab:top5_heads}
\end{table}

\subsubsection{End-to-End Evaluation}

Table~\ref{tab:end_to_end} compares all methods on the TriviaQA test set (200 samples). Figure~\ref{fig:method_comparison} visualizes the results.
...
\midrule
PPL              & .615 & .593 & .368 & .454 & .603 \\
SAPLMA-L16       & .720 & .701 & .621 & .659 & .767 \\
\midrule
Single L16\_H6   & .695 & .703 & .517 & .596 & .732 \\
Single L15\_H6   & .690 & .651 & .621 & .635 & .741 \\
Single L11\_H1   & .700 & .655 & .655 & .655 & .757 \\
Single L18\_H0   & .655 & .661 & .425 & .517 & .716 \\
Single L13\_H11  & .685 & .622 & .701 & .659 & .768 \\
\midrule
Summed-Heads     & .735 & .743 & .598 & .662 & .792 \\
Voting Ensemble  & \textbf{.735} & \textbf{.736} & .609 & \textbf{.667} & \textbf{.794} \\
\bottomrule
\end{tabular}
\caption{End-to-end hallucination detection on the TriviaQA test set (N=200). The Voting Ensemble achieves the best AUC (0.794) and the Summed-Heads probe achieves the best Accuracy (0.735).}
...
Key observations:
\begin{itemize}
    \item Both \textbf{Summed-Heads} (AUC 0.792) and \textbf{Voting Ensemble} (AUC 0.794) surpass SAPLMA (AUC 0.767) using only 5 of 336 heads' influence vectors, demonstrating that head-level features can be more discriminative than the full hidden state.
    \item \textbf{Decision-level fusion} (Voting, AUC 0.794) performs on par with \textbf{feature-level fusion} (Summed-Heads, AUC 0.792), suggesting the head signals are complementary and both fusion strategies are viable.
    \item \textbf{PPL} continues to show weak recall (0.368), struggling to detect hallucinated outputs, though its precision of 0.593 indicates moderate reliability on positive predictions.
    \item Among single-head probes, \textbf{L13\_H11} achieves the highest AUC (0.768), already matching the full SAPLMA baseline (0.767), while \textbf{L11\_H1} achieves the best balance (Acc 0.700, F1 0.655).
\end{itemize}
...
\begin{enumerate}
    \item SAPLMA (AUC 0.843) significantly outperforms PPL (AUC 0.690), confirming that hidden states contain richer truth-discriminative information than output probabilities.
    \item The optimal detection layer is Layer 16 (57\% depth), not the final layer, and the last token position is critical for effective detection (AUC 0.844 at Layer 16).
    \item A novel per-head probe method, using only 5 of 336 attention heads' influence vectors, achieves AUC 0.794 on a TriviaQA test set---surpassing the full SAPLMA baseline (AUC 0.767) with 300$\times$ fewer features.
    \item The most discriminative heads cluster in layers 11--18, providing interpretable evidence for the localization of hallucination-related signals in Transformer models.
\end{enumerate}
